...
- `npm test` → **356 passing / 0 fail / 2 skip** (node:test). The skips are the two Postgres suites without `DATABASE_URL` (expected; with it set → 360/0/0 incl. an engine-on-SQL booking e2e). Keep it green; add tests with every feature. `test/integration.e2e.test.js` proves the whole product: booking→owner alert, staff takeover, emergency override+alert+SSE, hot-lead alert+SSE.
