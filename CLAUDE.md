...
- `npm test` → **262 passing / 0 fail / 1 skip** (node:test). The skip is `test/store.postgres.test.js` without `DATABASE_URL` (expected). Keep it green; add tests with every feature. `test/integration.e2e.test.js` proves the whole product: booking→owner alert, staff takeover, emergency override+alert+SSE, hot-lead alert+SSE.
