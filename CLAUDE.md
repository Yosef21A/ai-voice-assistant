...
- `npm run simulate` → scripted AR + FR clinic bookings, a CABINET booking (doctor persona, never asks the specialty), a FACILITATOR qualification (rich lead + same-day-offer promise), PLUS an emergency-guardrail showcase (the detector overrides the engine reply and the bot steps back).
- `npm test` → **530 passing / 0 fail / 2 skip** (node:test). The skips are the two Postgres suites without `DATABASE_URL` (expected; with it set they run too, incl. an engine-on-SQL booking e2e). Keep it green; add tests with every feature. `test/integration.e2e.test.js` proves the whole product: booking→owner alert, staff takeover, emergency override+alert+SSE, hot-lead alert+SSE.
- **Voice tier (2026-07-31): the agent ANSWERS WhatsApp CALLS.** `src/voice-call/` (signaling + werift media) + `src/voice-call/brain/` (Gemini Live loop; deterministic two-phase spoken booking gate; same emergency detector as chat; degrade-to-chat). Calling is ENABLED on the test number (Path A). Spec + state: `docs/VOICE-AGENT-SPEC.md`. Voice guardrails are enforced in code (tools + predicates), never prompt-only.
...
- `src/llm/` — provider interface (`generate`, `generateStructured`, optional `transcribeAudio`); engine works 100% deterministically without any API key.
- `src/voice/` — V1 voice notes: `policy.js` (byte caps + honest transcript grading), `transcriber.js` (orchestration, degradation, quota breaker). `src/engine/voiceSlots.js` is the confirm-before-lock gate.
